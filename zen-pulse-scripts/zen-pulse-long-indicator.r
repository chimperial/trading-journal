//@version=5
indicator("(Long) Zen Pulse Indicator by ChillBits", overlay=true)
...
// === Signal logic for indicator (no trading logic) ===
...
alertcondition(entry_condition, title="Zen Pulse Long Entry", message="Zen Pulse Long Entry Signal: All conditions met for a long position.")

plot(entry_price, title="Potential Entry Price", color=color.green, linewidth=2, style=plot.style_circles)
plot(stop_price, title="Potential Stop Loss", color=color.red, linewidth=2, style=plot.style_circles)
plot(tp, title="Potential Take Profit", color=color.blue, linewidth=2, style=plot.style_circles)
